...
A = np.array(
    [
        [0, 1, 0, 0], 
        [0, (-1.0 / sigma) * ( (Bm / (r ** 2 * (m1 + m2))) + (kt*kb)/(R * r ** 2 * (m1+m2)) + (b/(m1 + m2)) ), (-0.5 / sigma) * (m2 * L) ** 2 * g / ((m1 + m2) * (I2 + m2 * L ** 2)), 0],
        [0, 0, 0, 1],
        [0, (-1.0 / sigma) * ( m2 * L) / (I2 + m2 * L ** 2) * ( (Bm / (r ** 2 * (m1 + m2))) + (kt * kb) / (R * r ** 2 * (m1 + m2)) + (b / (m1 + m2)) ), - 1 * (0.5 * (m2 * g * L) / (I2 + m2 * L ** 2) + ( (0.5 /sigma) * ((m2 * L) / (I2 + m2 * L ** 2)) * (m2 * L) ** 2 * g / ((m1+m2) * (I2 + m2 * L ** 2)) ) ), 0]
    ]
)

B = np.array([
    0, 
    (1.0 / sigma) * kt / ((m1 + m2) * R * r),
    0,
    (1.0 / sigma) * (m2 * L) / (I2 + m2 * L ** 2) * ( kt / ((m1 + m2) * R * r) )
]).reshape(-1, 1)
